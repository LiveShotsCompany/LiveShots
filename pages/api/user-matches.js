...
export default async function handler(req, res) {
    try {
        ...
        switch (method) {
            case "POST":
                const rows = await pool.query("SELECT matchId FROM UserFavoriteMatches WHERE userId = ?", [userId]);

                const favoriteMatches = rows.flatMap(row => row.map(entry => entry.matchId));
